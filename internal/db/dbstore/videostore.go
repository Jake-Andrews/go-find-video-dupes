...
func (r *videoRepo) GetVideos(ctx context.Context) ([]models.Video, map[int64][]models.Videohash) {
	log.Println("Getting videos from the database!")
	rows, err := r.db.QueryContext(ctx, `
		SELECT videoID, path, fileName, createdAt, modifiedAt, frameRate, videoCodec, audioCodec, width, height, duration, size, bitRate
		FROM video`)
	if err != nil {
		log.Fatalf("Error getting videos from database, err: %v", err)
	}
	defer rows.Close()
	...
	if err != nil {
		log.Fatalf("Get video hashes: %v", err)
	}
